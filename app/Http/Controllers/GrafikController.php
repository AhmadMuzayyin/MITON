...
class GrafikController extends Controller
{
    ...
    public function realFisik(){
        if (request()->all() == null) {
            $target = Target::where('pak_id', session()->get('pak_id'))->get()->groupBy('month_id');
            $opd = User::where('isAdmin', 0)->get();
            $opdmelapor = Report::where('pak_id', session()->get('pak_id'))->where('status', 1)->get()->groupBy('month_id');
            $data = [
                'target' => $target,
                'opdmelapor' => $opdmelapor,
                'opd' => $opd
            ];
            return response()->json($data);
        }
            $target = Target::where('pak_id', session()->get('pak_id'))->where('month_id', request()->get('bulan'))->get()->groupBy('month_id');
            $opd = User::where('isAdmin', 0)->get();
            $opdmelapor = Report::where('pak_id', session()->get('pak_id'))->where('status', 1)->where('sumber_dana_id', request()->get('dana'))->where('month_id', request()->get('bulan'))->get()->groupBy('month_id');
            $data = [
                'target' => $target,
                'opdmelapor' => $opdmelapor,
                'opd' => $opd
    ...
    public function realKeuangan(){
        if (request()->all() == null) {
            $target = TKeuangan::where('sumber_dana_id', 1)->where('pak_id', session()->get('pak_id'))->where('kondisi', session()->get('kondisi'))->get();
            $anggaran = Anggaran::where('pak_id', session()->get('pak_id'))->where('kondisi', session()->get('kondisi'))->get();
            $realanggaran = Report::where('pak_id', session()->get('pak_id'))->where('kondisi', session()->get('kondisi'))->where('status', 1)->where('sumber_dana_id', 1)->get();
            $data = [
                'target' => $target,
        ...
        $target = TKeuangan::where('sumber_dana_id', request()->get('dana'))->where('month_id', request()->get('bulan'))->where('pak_id', session()->get('pak_id'))->where('kondisi', session()->get('kondisi'))->get();
        $anggaran = Anggaran::where('pak_id', session()->get('pak_id'))->where('kondisi', session()->get('kondisi'))->get();
        $realanggaran = Report::where('pak_id', session()->get('pak_id'))->where('kondisi', session()->get('kondisi'))->where('status', 1)->where('sumber_dana_id', request()->get('dana'))->where('month_id', request()->get('bulan'))->get();
        $data = [
            'target' => $target,
            'realanggaran' => $realanggaran,
            'anggaran' => $anggaran

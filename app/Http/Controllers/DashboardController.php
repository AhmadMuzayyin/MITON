...
class DashboardController extends Controller
{
    ...
    public function indexData(){
        if (Auth()->user()->isAdmin != 1) {
          ...
          return \response()->json([
            'realisasi' => array_map(function($n) { return \floatval($n); }, array_column($data, 'keuangan')), 
            'target' => array_map(function($n) { return \floatval($n); }, array_column($target, 'keuangan'))
          ]);
        }
    ...
    public function indexData2(){
        if(Auth()->user()->isAdmin != 1){
          ...
          return \response()->json([
            'realisasi' => array_map(function($n) { return \floatval($n); }, array_column($data, 'persentase')), 
            'target' => array_map(function($n) { return \floatval($n); }, array_column($target, 'persentase'))
          ]);
        }
     ...
     try {
      //  dd($request->all(), time().'_'.$request->file('foto')->getClientOriginalName());
       if ($request->file('foto') != null) {
        ...
        return back()->with('success', 'Berhasil merubah data profil andan.');
     } catch (\Throwable $th) {
